...
#define OEMIROT_AREA_1_SIZE             (0x3A000)       /* 232K */
#define OEMIROT_AREA_2_SIZE             (0x14000)       /* 80K */
#define OEMIROT_AREA_2_OFFSET           (0x6A000)       /* slot 2 address when flash size: 512 KBytes */
...
#elif defined(OEMUROT_ENABLE) && !defined(MCUBOOT_OVERWRITE_ONLY)
/* For swap mode and uRoT, the NS partition is limited to 48 KB, unless FULL SECURE is enabled. */
#define FLASH_NS_PARTITION_SIZE         (0xC000)

#else
/* For Others case, we limit the size to 64 KB for NS partition (except for FULL SECURE) */
#define FLASH_NS_PARTITION_SIZE         (0x10000)
