...
$python$applicfg flash --layout $preprocess_bl2_file -b wrp1a_start -m RE_BL2_WRP_START -d 0x1000 $ob_flash_programming --vb >> $current_log_file 2>&1
ret=$?
if [ $ret != 0 ]; then error; fi

$python$applicfg flash --layout $preprocess_bl2_file -b wrp1a_end -m RE_BL2_WRP_END -d 0x1000 $ob_flash_programming --vb >> $current_log_file 2>&1
ret=$?
if [ $ret != 0 ]; then error; fi

if [ "$oemurot_enabled" == "1" ]; then
  $python$applicfg flash --layout $preprocess_bl2_file -b wrp1b_start -m RE_BL2_OEMUROT_PERSO_WRP_START -d 0x1000 $ob_flash_programming --vb >> $current_log_file 2>&1
  ret=$?
  if [ $ret != 0 ]; then error; fi

  $python$applicfg flash --layout $preprocess_bl2_file -b wrp1b_end -m RE_BL2_OEMUROT_PERSO_WRP_END -d 0x1000 $ob_flash_programming --vb >> $current_log_file 2>&1
  ret=$?
  if [ $ret != 0 ]; then error; fi
fi
...
$python$applicfg flash --layout $preprocess_bl2_file -b sec1_end -m RE_BL2_SEC1_END -d 0x1000 $ob_flash_programming --vb >> $current_log_file 2>&1
ret=$?
if [ $ret != 0 ]; then error; fi

$python$applicfg flash --layout $preprocess_bl2_file -b sec2_start -m RE_BL2_SEC2_START -d 0x1000 $ob_flash_programming --vb >> $current_log_file 2>&1
ret=$?
if [ $ret != 0 ]; then error; fi

$python$applicfg flash --layout $preprocess_bl2_file -b sec2_end -m  RE_BL2_SEC2_END -d 0x1000 $ob_flash_programming --vb >> $current_log_file 2>&1
ret=$?
if [ $ret != 0 ]; then error; fi

$python$applicfg flash --layout $preprocess_bl2_file -b hdp1_end -m RE_BL2_HDP_END -d 0x1000 $ob_flash_programming --vb >> $current_log_file 2>&1
ret=$?
if [ $ret != 0 ]; then error; fi

$python$applicfg flash --layout $preprocess_bl2_file -b hdp2_end -m RE_BL2_HDP_END -d 0x1000 $ob_flash_programming --vb >> $current_log_file 2>&1
ret=$?
if [ $ret != 0 ]; then error; fi
