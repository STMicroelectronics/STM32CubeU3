                                 ...
                                 const void *data, uint32_t cnt)
{
  ...
  */
  ...
  if (err != HAL_OK)
  {
    ...
    printf("->>> Failed to write 0x%" PRIx32 " bytes at 0x%" PRIx32 " (Error 0x%" PRIx32 ")", cnt, (uint32_t)FLASH_BASE + addr, flashError);
  }
...
static int32_t Flash_EraseSector(uint32_t addr)
{
  ...
  {
    flashError = HAL_FLASH_GetError();
    printf("->>> Erase failed at 0x%" PRIx32 " (Error 0x%" PRIx32 ")", (uint32_t)FLASH_BASE + addr, flashError);
  }
