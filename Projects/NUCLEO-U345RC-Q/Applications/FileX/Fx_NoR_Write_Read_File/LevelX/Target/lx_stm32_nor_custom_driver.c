...
UINT  lx_stm32_nor_custom_driver_initialize(LX_NOR_FLASH *nor_flash)
{
  ...
  /* get the memory part characteristics */
   w25q128j_status = w25q128j_Init(&hspi1);
...
/**
  ...
  * @param  max_timeout : Maximum timeout value used to poll the NOR status
...
static inline UINT nor_flash_check_status(SPI_HandleTypeDef *phspi, ULONG max_timeout)
{
  ...
  while (HAL_GetTick() - start < max_timeout)
  {
    ...
    if ((w25q128j_status == W25Q128J_OK) && (p_status == W25Q128J_OK))
    {
      return LX_SUCCESS;
    }
  }
...
static UINT lx_nor_driver_read(ULONG *flash_address, ULONG *destination, ULONG words)
{
  ...
  ret = nor_flash_check_status(&hspi1, nor_flash_timeout);
  if (ret != LX_SUCCESS)
  {
    return ret;
  }
  else
  {
    ...
    w25q128j_status = w25q128j_Read(&hspi1, (uint8_t *) destination, (uint32_t)flash_address, words * sizeof(ULONG));
    if (w25q128j_status != W25Q128J_OK)
    {
      return LX_ERROR;
    }
...
static UINT lx_nor_driver_write(ULONG *flash_address, ULONG *source, ULONG words)
{
  ...
  else
  {
    ...
    w25q128j_status = w25q128j_Write(&hspi1, (uint8_t *) source,
...
static UINT lx_nor_driver_block_erase(ULONG block, ULONG erase_count)
{
  ...
  else
  {
    ...
    w25q128j_status = w25q128j_Erase(&hspi1, block, W25Q128J_ERASE_64K_BLOCK);
    if (w25q128j_status != W25Q128J_OK)
    {
      return LX_ERROR;
    }
...
static UINT lx_nor_driver_block_erased_verify(ULONG block)
{
  ...
  {
    w25q128j_status = w25q128j_Read(&hspi1, (uint8_t *)nor_flash_read_buffer, (uint32_t)start_address, LX_NOR_SECTOR_SIZE * sizeof(ULONG));
    if (w25q128j_status != W25Q128J_OK)
    {
      return LX_ERROR;
    }
