...
It relies on cryptography hardware peripherals for optimal performances (SAE, HASH, RNG).
...
### <b>Hardware and Software environment</b>
  ...
  - This example runs on STM32U345xx devices with security enabled (TZEN=B4).
  - This example has been tested with STMicroelectronics NUCLEO-U345RC-Q (MB1841)
...
the provisioning process for OEMiROT boot path.
...
External tamper is TAMP_IN1 (PC13, user button) on NUCLEO-U345RC-Q board.
...
For STM32U345xx devices, with all IDE's, the are the following limitation:

   - RDP level 0.5 not supported.
   - OEMuRoT not supported.
