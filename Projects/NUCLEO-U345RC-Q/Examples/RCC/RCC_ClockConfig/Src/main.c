...
static void SwitchSystemClock(void)
{
  if (__HAL_RCC_GET_SYSCLK_SOURCE() == RCC_SYSCLKSOURCE_STATUS_MSIS)
  {
...
static void SystemClockHSI_Config(void)
{
  ...
  /* -4- Optional: Disable MSIK / MSIS Oscillator */
...
static void SystemClockMSIS_Config(void)
{
  ...
  /* -1- Select HSI as system clock source to allow modification */
  RCC_ClkInitStruct.ClockType       = RCC_CLOCKTYPE_SYSCLK;
  RCC_ClkInitStruct.SYSCLKSource    = RCC_SYSCLKSOURCE_HSI;
  if (HAL_RCC_ClockConfig(&RCC_ClkInitStruct, FLASH_LATENCY_0) != HAL_OK)
  {
  ...
  /* -4- Optional: Disable MSIK / HSI Oscillator */
