...
int main(void)
{
  ...
  if (__HAL_PWR_GET_FLAG(PWR_FLAG_SBF) == 1U)
  {
    ...
    if (HAL_PWR_GetClearWakeupSource() == PWR_WAKEUP_LINE9)
    {
  ...
  {
    /* Enable the wake up line 9 for I3C1 */
    HAL_PWR_EnableWakeUpLine(PWR_WAKEUP_LINE9, PWR_WAKEUP_SELECT_0, PWR_WAKEUP_POLARITY_LOW);

    /* Enable ULPMEN bit for lower consumption */
    HAL_PWREx_EnableUltraLowPowerMode();

    /* Enable I3C pull-up */
    HAL_PWREx_EnableI3CPullUp(PWR_I3CPU_PA7);
    HAL_PWREx_EnableI3CPullUp(PWR_I3CPU_PA6);
