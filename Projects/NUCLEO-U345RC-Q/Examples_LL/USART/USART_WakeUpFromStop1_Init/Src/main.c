...
void     PrintInfo(void);
static void RestoreReferenceRunClock(void);
...
static void RestoreReferenceRunClock(void)
{
  ...
  while (LL_PWR_IsActiveFlag_REGULATOR() == 0U)
  {
  }
  ...
  LL_Init1msTick(3000000U);
  LL_SetSystemCoreClock(3000000U);
}
...
int main(void)
{
  ...
  LL_RCC_SetStopWakeupClock(LL_RCC_STOP_WAKEUP_CLK_MSIS);
...
*/
  ...
  LL_Init1msTick(3000000);

  LL_SetSystemCoreClock(3000000);
}
...
static void MX_USART1_UART_Init(void)
{
  ...
  LL_RCC_SetUSARTClockSource(LL_RCC_USART1_CLKSOURCE_HSI16);
...
void Configure_PWR(void)
{
  ...
  /* Ensure that MSIS is wake-up system clock */
  LL_RCC_SetStopWakeupClock(LL_RCC_STOP_WAKEUP_CLK_MSIS);
}
