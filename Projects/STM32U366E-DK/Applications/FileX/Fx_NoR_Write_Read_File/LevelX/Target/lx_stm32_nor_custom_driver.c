...
static UINT lx_nor_driver_read(ULONG *flash_address, ULONG *destination, ULONG words)
{
  ...
  else
  {
    ...
    w25q128j_status = w25q128j_Read(&hspi2, (uint8_t *) destination, (uint32_t)flash_address, words * sizeof(ULONG));
    if (w25q128j_status != W25Q128J_OK)
    {
      return LX_ERROR;
    }
...
static UINT lx_nor_driver_write(ULONG *flash_address, ULONG *source, ULONG words)
{
  ...
  else
  {
    ...
    w25q128j_status = w25q128j_Write(&hspi2, (uint8_t *) source,
...
static UINT lx_nor_driver_block_erase(ULONG block, ULONG erase_count)
{
  ...
  else
  {
    ...
    w25q128j_status = w25q128j_Erase(&hspi2, block, W25Q128J_ERASE_64K_BLOCK);
    if (w25q128j_status != W25Q128J_OK)
    {
      return LX_ERROR;
    }
...
static UINT lx_nor_driver_block_erased_verify(ULONG block)
{
  ...
  {
    w25q128j_status = w25q128j_Read(&hspi2, (uint8_t *)nor_flash_read_buffer, (uint32_t)start_address, LX_NOR_SECTOR_SIZE * sizeof(ULONG));
    if (w25q128j_status != W25Q128J_OK)
    {
      return LX_ERROR;
    }
