...
int main(void)
{
  ...
  __HAL_LINKDMA(&hi2c3, hdmatx, handle_GPDMA1_Channel7);
  MX_Queue_rx_Config();
  HAL_DMAEx_List_LinkQ(&handle_GPDMA1_Channel6, &Queue_rx);
  __HAL_LINKDMA(&hi2c3, hdmarx, handle_GPDMA1_Channel6);

  /* Disable debug in STOP mode to get the real low-power behavior */
  HAL_DBGMCU_DisableDBGStopMode();

  /* The board sends the message automatically after the RTC wakeup trigger
     fires while the CPU is in STOP2 mode, then expects to receive it back */
  ...
  do
  {
    if(HAL_I2C_Master_Transmit_DMA(&hi2c3, (uint16_t)I2C_ADDRESS, (uint8_t*)aTxBuffer, TXBUFFERSIZE)!= HAL_OK)
    {
    ...
    if (HAL_RTCEx_SetWakeUpTimer_IT(&hrtc, 2048, RTC_WAKEUPCLOCK_RTCCLK_DIV16, 0) != HAL_OK)
    {
      Error_Handler();
    }

    /* Enter STOP2 and let RTC_WUT autonomously trigger the I2C transfer */
    HAL_SuspendTick();
  ...
  while((HAL_I2C_GetError(&hi2c3) == HAL_I2C_ERROR_AF) && (i2c_af_retry < 1000U));

  if (i2c_af_retry >= 1000U)
  {
    Error_Handler();
  }
  ...
  do
  {
    if(HAL_I2C_Master_Receive_DMA(&hi2c3, (uint16_t)I2C_ADDRESS, (uint8_t *)aRxBuffer, RXBUFFERSIZE) != HAL_OK)
    {
      /* Error_Handler() function is called when error occurs. */
      Error_Handler();
    }
...
static void MX_I2C3_Init(void)
{
  ...
  */
  ...
  sConfigI2C3.TriggerSelection = I2C_GRP2_RTC_WUT_TRG;
