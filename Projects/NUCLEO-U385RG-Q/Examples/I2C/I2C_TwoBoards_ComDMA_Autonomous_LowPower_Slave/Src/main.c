...
int main(void)
{
  ...
  if(HAL_I2C_Slave_Receive_DMA(&hi2c3, (uint8_t *)aRxBuffer, RXBUFFERSIZE) != HAL_OK)
  {
    /* Transfer error in reception process */
    Error_Handler();
  }

  /* Keep address match interrupt enabled: it is the wake-up event when the
     master sends the slave address while this board is in STOP2 */
  __HAL_I2C_ENABLE_IT(&hi2c3, I2C_IT_ADDRI);
  __HAL_I2C_ENABLE_IT(&hi2c3, I2C_IT_STOPI);

  /* Select the kernel clock restored on STOP exit so the I2C wake-up path
     remains aligned with the I2C3 MSIK clock source used by this example */
  HAL_RCCEx_StopWakeupKernelClkConfig(RCC_KERNELCLK_MSIK);
  ...
  /* While the I2C in reception process, user can transmit data through
     "aTxBuffer" buffer */
  if(HAL_I2C_Slave_Transmit_DMA(&hi2c3, (uint8_t*)aTxBuffer, TXBUFFERSIZE)!= HAL_OK)
  {
